...
public class CourseLogTransactionDAO extends BaseDAO {
    ...
    public int update(CourseLogTransaction courseLogTransaction) throws SQLException {
        try {
            this.query = String.format(
                    "UPDATE %s " +
                    "SET buyer = %s, seller = %s, course_id = %s, "
                            + "course_price = %s, cashback_percentage = %d, amount_cashback = %s, payment_status = %d " +
                    "WHERE id = %d ",
                    this.nomeTabela,
                    courseLogTransaction.getBuyer(),
                    courseLogTransaction.getSeller(),
